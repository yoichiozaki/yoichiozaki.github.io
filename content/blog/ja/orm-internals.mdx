...
SQLAlchemy のレガシー構文である `session.query(User).filter(User.age > 20).all()` のように書くだけで、SQLが生成され、データベースから取得した行がPythonオブジェクトに変換される——ORMは「魔法」のように見えますが、その裏側には明確な設計原理があります。
...
# 全ユーザーを取得: 1回のSELECT（SQLAlchemy 2.x style）
users = session.scalars(select(User)).all()  # SELECT * FROM users (1回)
...
以下の例では、`parent_table`、`child_table`、`fk_column` が信頼済みの ORM メタデータから来る前提にしています。SQL のバインドパラメータで守れるのは値であって識別子ではないため、実際の ORM はテーブル名やカラム名をユーザー入力として受け取らず、検証・クォートしてから SQL に埋め込みます。
...
EF Core は C# のLINQ（Language Integrated Query）を使います。`DbSet<T>` に対するクエリは `IQueryable<T>` なので、LINQ のラムダ式が<strong>式木（Expression Tree）</strong>として捕捉され、EF Core がその式表現をSQLに変換します。Pythonの演算子オーバーロードによるSQL式生成と同じ原理ですが、言語レベルでサポートされている点が異なります。
...
for user in session.scalars(select(User).execution_options(yield_per=1000)):
    process(user)
    # yield_per は結果全体を一度に実体化せず、行をバッチ単位でフェッチする
