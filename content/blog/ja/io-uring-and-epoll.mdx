...
ここで言う <strong>syscall (システムコール)</strong> は、ユーザ空間のプログラムがカーネル機能を呼び出すための特権モード切替のこと。トランポリン命令と CPU モード遷移を伴うため、単純な関数呼び出しとは桁違いに重い処理です。単純な syscall が 200–500 ns 程度、普通の関数呼び出しが < 10 ns 程度という数字はあくまでオーダー感で、実際のコストは CPU・カーネル・緩和策・キャッシュ/TLB 状態に大きく依存します。後述の KPTI でさらに重くなることがあります。
...
- <strong>`EPOLLEXCLUSIVE`</strong> (Linux 4.5+) — epoll の内部 wait_queue エントリに `WQ_FLAG_EXCLUSIVE` を立て、全 waiter を起こす代わりに 1 つまたは少数の exclusive waiter だけを起こして thundering herd を抑える。
...
<strong>要するに</strong>: SQE が「依頼票」、CQE が「その控え」で、`user_data` は両者を紐づける背番号。アプリ側は「いくつ依頼が出ていて」「それぞれが何か」を `user_data` で記憶しておき、戻ってきた CQE の `user_data` で元のコンテキストを引く。
...
ここで重要なのは、複数 SQE を <strong>1 回の `io_uring_enter` でまとめて送れる</strong> こと。1 syscall で 1000 個の read を投げられるので、syscall コストが償却されます。これは移植可能なベンチマーク値ではなく概算ですが、epoll 風のループでは 1 回の I/O に syscall 2 回を払う一方、io_uring では 1000 依頼をバッチして 1 回の submit syscall にまとめられます。
...
`io_uring_setup` に `IORING_SETUP_SQPOLL` を立てると、カーネルが <strong>専用スレッド</strong> を作って SQ を監視し続けます。そのスレッドが起きていてリングが忙しい間は、ユーザ空間は共有メモリに SQE を置くだけで `io_uring_enter` なしに submit / reap できます。ただし SQ スレッドがアイドルで眠り `IORING_SQ_NEED_WAKEUP` を立てた場合は、起こすための `io_uring_enter` が必要です。つまり「カウンターに専任の担当者を常駐させて、依頼票を置いたそばから取りに行かせる。ただし暇が続くと仮眠に入る」モードです。
...
`O_DIRECT` のブロック I/O で、ファイルシステムとデバイスが polling をサポートしている場合、カーネル内でブロックレイヤの完了を割込みでなく <strong>ビジーポーリング</strong> で取りに行けます。NVMe の μs オーダーのレイテンシではこれが優位になることがあります。polled completion の回収にも `io_uring_enter` を使います。
...
> <strong>一言でいうと</strong>: 強力だが脆弱性報告も多く、seccomp では opcode 単位に制御できない。本番で使うなら最新 LTS カーネル + 観測ツール + hardened platform / ディストリビューションのポリシー確認が必須。
...
- その結果、<strong>Android や ChromeOS など一部の hardened platform は `io_uring` を既定で無効化または制限</strong>している。汎用サーバーディストリビューションの方針はまちまちなので、実環境のポリシー確認が必要。
- RHEL 9.5 以降、`sysctl kernel.io_uring_disabled=1/2` で全面禁止 / root のみ許可が選べるようになった。

プロダクションで `io_uring` を使う場合、<strong>カーネルを常に最新 LTS にする</strong>こと、必要に応じて seccomp で `io_uring_*` syscall 自体を禁止・制限すること、opcode 単位の制限が必要なら `IORING_REGISTER_RESTRICTIONS` や LSM ベースの制御を使うことが重要です。
...
- リングごとの情報: 対応カーネルでは `/proc/<pid>/fdinfo/<ring-fd>` に io_uring の fdinfo が出る。ライブ調査では tracepoint の方が有用なことが多い。
