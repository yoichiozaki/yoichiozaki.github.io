...
6. <strong>デメリットと注意点</strong> — API の安定性・非決定性・セキュリティ
...
> <strong>追記（2026-09-10 時点）</strong>：本記事執筆時点では `HarnessAgent` は実験的（experimental / preview）な API で、コンパイル時に診断 ID `MAAI001` の警告が出ていた。その後これは解消され、現在の `HarnessAgent` は<strong>安定版の `Microsoft.Agents.AI.Harness` パッケージ（本稿更新時点で 1.20.0）で提供される正式 API</strong> であり、公式ドキュメントでも Agent Framework の基礎概念（Agents / Workflows / <strong>Agent Harness</strong>）の一つとして位置づけられている。以降のコード例にある `--prerelease` や `MAAI001` の抑制は、ハーネス自体についてはもう不要だ。ただし `MAAI001` 自体は Agent Framework 全体の実験的機能向け診断 ID として存続しており、ファイル記憶、A2A、Mem0、DevUI、宣言的エージェント、シェル実行などを使う場合は依然として抑制が必要になる。
...
| <strong>言語</strong> | Node/Python/Go/.NET/Java/Rust | .NET が最も成熟（Python も正式提供、Go は public preview） |
| <strong>配布</strong> | CLI ランタイム同梱 | NuGet ライブラリ（安定版） |

最も本質的な違いは<strong>能力の絞り込み方</strong>だ。Copilot SDK は「強力なランタイムに対し、実行時に一つひとつ許可を出す（または拒否する）」という<strong>動的なゲート</strong>で安全性を担保した。一方ハーネスは「どの能力を組み込むかを<strong>構成時に</strong>選ぶ」という<strong>静的な組み立て</strong>で安全性を担保する。`DisableWebSearch = true` のように要らない組み込み能力を外し、`FileAccessStore` のような opt-in 能力は最初から渡さないわけだ。
...
依存パッケージを追加する。執筆時点では Agent Harness が実験的だったため `--prerelease` が必要だった（現在は `Microsoft.Agents.AI.Harness` に安定版があるので、ハーネスのためだけにこのフラグを付ける必要はない）。
...
執筆時点の `HarnessAgent` は実験的 API だったので、`.csproj` で警告を抑制しておく（現行版では `MAAI001` の抑制はハーネスのためだけには不要。他の実験的機能を使う場合は引き続き必要）。
...
文書レビューに、ハーネスが標準で持つ Web 検索・ファイル記憶・スキル探索は要らない。任意のファイル IO は `FileAccessStore` を渡した場合だけ有効になるため、この例では最初から構成しない。むしろ、信頼できない文書を読む以上、<strong>攻撃面は最小化すべき</strong>だ。そこで `Disable*` フラグでそれらを外し、<strong>計画機能（TodoProvider・AgentModeProvider）と自前のツールだけ</strong>を残す。
...
AIAgent CreateReviewer(IChatClient client, Action<string, string> emit) =>
    client.AsHarnessAgent(new HarnessAgentOptions
    {
        ...
        DisableToolAutoApproval = true,   // 承認が必要なツールを使わないため自動承認ミドルウェアは外す

        // TodoProvider と AgentModeProvider（plan/execute）は残す → 計画機能を活かす
        // FileAccessStore と ShellExecutor は設定しない → 任意ファイル IO とシェル実行は最初から不可能
...
`FileAccessStore` と `ShellExecutor` を設定していないので、<strong>任意ファイル IO とシェル実行はそもそも構成されない</strong>。`DisableToolAutoApproval = true` にしているのは、承認必須のツールを使わず、サーバ側では自前ツールだけに絞るためだ。その代わり、<strong>提供する能力自体を安全なものだけに絞る</strong>ことで安全性を担保している——これがハーネス流の防御だ。

これらのフラグが「何を制御し、何を制御しないか」は明確にしておきたい。`Disable*` フラグが無効化するのは、ハーネスの<strong>組み込みプロバイダ</strong>（`HostedWebSearchTool`、`FileMemoryProvider` など）だ。`FileAccessProvider` はそもそも `FileAccessStore` を明示的に渡した場合だけ追加される。自分のツールには<strong>影響しない</strong>——`save_review` は依然としてファイルを書き、`check_links` は依然として外部へアクセスする。これらはハーネスのプロバイダではなく、自分の `AIFunction` だからだ。Copilot SDK 版と同じく、カスタムツールにとっての本当の防御線は<strong>ツールの内側</strong>（パストラバーサル対策・タイムアウト・許可ドメイン）にある。

> <strong>能力の最小化こそがハーネスの防御線。</strong> もし `ShellExecutor` や `FileAccessStore` を渡したり、`DisableWebSearch` を外したりすれば、エージェントは強力になるが攻撃面も一気に広がる。信頼できない入力（レビュー対象の文書）を読ませる以上、「何を組み込まないか」がセキュリティ設計の中心になる。
...
### API の安定性（執筆時は実験的 API だった）

執筆時点の `HarnessAgent` は実験的（`MAAI001`）で、Copilot SDK が GA（一般提供）なのと対照的だった。<strong>この差はすでに埋まっている</strong>。現在の `HarnessAgent` は安定版 `Microsoft.Agents.AI.Harness`（本稿更新時点で 1.20.0）で提供され、`[Experimental]` 属性も付いていない。

ただし、「ハーネス本体が安定した」ことと「周辺機能すべてが安定した」ことは別だ。.NET 側では A2A、Mem0、DevUI、宣言的エージェント、シェルツールなどが依然 `MAAI001` の対象であり、Python 側でも background agents・file access・looping は実験的、シェルツールはプレリリースパッケージのままだ。本番採用では「どの機能を使うか」単位で安定性を確かめるのが良い。
...
- <strong>プロンプトインジェクション</strong> — 文書内の悪意ある指示への対策は、ハーネスでは<strong>能力の最小化</strong>が中心になる。`FileAccessStore` や Web 検索を有効化したり `ShellExecutor` を渡したりするほど、インジェクションの被害は大きくなる。
- <strong>SSRF</strong> — `check_links` のような外部アクセスにはタイムアウトと許可ドメインを設ける。Web 検索やファイルアクセスを安易に有効化しないことも重要だ。
- <strong>承認モデルの選択</strong> — 本実装は非対話サーバなので `DisableToolAutoApproval = true` とし、能力の絞り込みで代替した。対話的な用途なら、逆にツール承認を有効にして人間に判断させる設計もありうる。<strong>「構成で絞る」か「実行時に承認する」か</strong>は、用途に応じて選ぶ。

### .NET 中心

最も成熟しているのは .NET 実装だ。Agent Framework のハーネスは Python でも正式に提供されており（`create_harness_agent`。リポジトリに [Python のハーネスサンプル](https://github.com/microsoft/agent-framework/tree/main/python/samples/02-agents/harness)もある）、Agent Framework 自体は Go 向け実装（public preview）も持つ。ただし Go には現時点でパッケージ化されたハーネスがなく、各パッケージを自分で組み上げる必要がある。本記事では検証済みの C# を用いた。多言語チームでは言語選択が制約になりうる。
...
| 選択肢 | 向いている場面 |
|--------|--------------|
| <strong>GitHub Copilot SDK</strong> | 多言語（特に Node/TS）で素早く。GitHub エコシステム/サブスクの中で。実行時の細かい権限制御がほしい。 |
...
4. <strong>代償は委譲共通</strong> — 非決定性・コスト・セキュリティという委譲共通の代償はここでも付いて回る。なお、執筆時にあった「ハーネスは実験的 API」という制約はすでに解消され、現在は安定版として提供されている。
