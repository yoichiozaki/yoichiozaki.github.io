...
> **追記（2026-08 時点）**：本記事執筆時点では `HarnessAgent` は実験的（experimental / preview）な API で、コンパイル時に診断 ID `MAAI001` の警告が出ていた。その後これは解消され、現在の `HarnessAgent` は<strong>安定版の `Microsoft.Agents.AI.Harness` パッケージ（本稿更新時点で 1.19.0）で提供される正式 API</strong> であり、公式ドキュメントでも Agent Framework の 4 つの柱（Agents / **Harness Agent** / Workflows / Integrations）の一つとして位置づけられている。以降のコード例にある `--prerelease` や `MAAI001` の抑制は、ハーネス自体についてはもう不要だ。ただし `MAAI001` 自体は Agent Framework 全体の実験的機能向け診断 ID として存続しており、A2A、Mem0、DevUI、宣言的エージェント、シェル実行などを使う場合は依然として抑制が必要になる。
...
執筆時点の `HarnessAgent` は実験的（`MAAI001`）で、Copilot SDK が GA（一般提供）なのと対照的だった。<strong>この差はすでに埋まっている</strong>。現在の `HarnessAgent` は安定版 `Microsoft.Agents.AI.Harness`（本稿更新時点で 1.19.0）で提供され、`[Experimental]` 属性も付いていない。
