...
Go の `func(*Server)` と C# の `Action<SmtpOptions>` は言語機能としては別物だが、この文脈では<strong>同じ設計意図</strong>を持つ。どちらも「Options オブジェクトを受け取り、そのフィールドを変更する関数」として設定処理を合成している。
...
|---------------|-----------|---------|------|
| `IOptions<T>` | Singleton | 不可（初回アクセス時に作成され、その後キャッシュ） | アプリ全体で変わらない設定 |
| `IOptionsSnapshot<T>` | Scoped | スコープごと（Web では通常リクエストごと） | リクエスト単位で再計算された値を取得。Singleton には注入しない |
| `IOptionsMonitor<T>` | Singleton | 変更通知と現在値の再読み込み | Singleton からも利用できるホットリロード対応 |
...
ASP.NET Core の Options 解決は [`OptionsFactory<TOptions>`](https://github.com/dotnet/runtime/blob/main/src/libraries/Microsoft.Extensions.Options/src/OptionsFactory.cs) が行う。`IOptions<T>` は生成した値をキャッシュし、`IOptionsSnapshot<T>` はスコープ内でキャッシュし、`IOptionsMonitor<T>` は変更通知と `IOptionsMonitorCache<T>` によって現在値を管理する。簡略化すると以下の流れだ。
