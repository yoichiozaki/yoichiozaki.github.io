...
[GraphRAG](https://github.com/microsoft/graphrag) は、Microsoft Research が開発した RAG 手法です。非構造化テキストから LLM を使って **知識グラフ**（ナレッジグラフ）を構築し、そのグラフ構造を活用してクエリ時のコンテキストを強化します。2026-09-10 時点で、`microsoft/graphrag` リポジトリは自身を「主にメンテナンスモード」と説明しており、活発に機能追加される製品 SDK ではありません。手法とドキュメントは有用ですが、本番利用前には現在のドキュメントでパッケージ API と設定形式を確認してください。
...
まず、新しいコンソールプロジェクトを作成し、現在の Agent Framework / OpenAI 関連パッケージを追加します。2026-09-10 時点の Agent Framework クイックスタートでは、C# 例は `Microsoft.Agents.AI` と OpenAI .NET SDK を組み合わせ、Azure OpenAI 認証には `Azure.Identity` と `System.ClientModel` を使います。以降のコードは GraphRAG 風の実装を説明するための例なので、本番コードへ転用する前に Agent Framework の正確な API を最新ドキュメントで確認してください。
...
GraphRAG のインデキシングパイプラインの Phase 4 にあたるコミュニティ検出を説明します。本家の GraphRAG は Python の `graspologic` ライブラリを使って Leiden アルゴリズムを実行しますが、ここで示す C# コードは、グラフクラスタリングの役割を説明するための簡易ローカル移動ヒューリスティックであり、Leiden の忠実な実装ではありません。

主な処理の流れは以下の通りです。

1. エンティティ間の関係性から隣接リスト（adjacency list）を構築
2. 各エンティティを初期状態として独自のコミュニティに配置
3. 各エンティティについて、隣接ノード数が最も多いコミュニティに移動する（説明用の簡易スコア）
4. 改善が見られなくなるまでループを繰り返す

```csharp
// Services/CommunityDetector.cs
// 説明用の簡易コミュニティ検出器。完全な Leiden 実装ではない

/// <summary>
/// 説明用の簡易階層的コミュニティ検出。
/// GraphRAG では graspologic (Python) を使用するが、
/// この C# 例は説明用の簡易ローカル移動ヒューリスティックのみを示す。
/// </summary>
public class CommunityDetector
{
        ...
        int maxCommunitySize = 10)
    {
        ...
        // 3. 説明用のローカル移動ループ（実際の Leiden は modularity/CPM をより厳密に最適化）
        bool improved;
        do
        {
            improved = false;
...
上記の実装は説明用のローカル移動ベースの簡易コミュニティ検出器であり、正しい Leiden 実装そのものではありません。本格的な Leiden 実装では、さらに以下の処理が含まれます。
...
プロダクション環境では、公式の Python GraphRAG / graspologic パイプラインでインデックスを作成して Parquet 出力を .NET から読むか、メンテナンスされている Leiden 実装を提供するグラフライブラリ/ランタイムを使うのが現実的です。
...
LazyGraphRAG:      NLP ベースのインデキシング、LLM 処理はすべてクエリ時に遅延実行（Microsoft Research の実験では、インデキシングコストはベクトル RAG と同等レベル、標準 GraphRAG の約 0.1% と報告）
```
...
GraphRAG の動作は YAML 設定ファイル（`settings.yaml`）で細かく制御できます。`graphrag init` コマンドでテンプレートが生成されます。

```yaml
# settings.yaml — 主要な設定パラメータ（graphrag init で生成）
completion_models:
