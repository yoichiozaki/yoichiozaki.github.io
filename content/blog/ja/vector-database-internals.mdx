...
LUT 構築後は<strong>1 回の距離計算に浮動小数乗算が 0 回</strong>です。$m=64$ なら、格納コードごとに 64 回のバイト ID によるテーブル参照と、テーブル値の加算で済みます。FAISS の `pq4_fast_scan` のような SIMD fast-scan カーネルは大きなバッチを高速に走査できますが、実スループットはコードサイズ、CPU の SIMD 幅、メモリ配置、再現率設定に依存します。

<strong>要するに</strong>: PQ の距離計算は「$d$ 次元の差分計算と二乗」から「あらかじめ作った表の足し算」にすり替わっており、これが PQ が桁違いに速い本質的な理由です。
...
1. <strong>Navigable Small World (NSW)</strong>: 近傍だけでなく長距離エッジも持つグラフは、貪欲ルーティング (各ステップで『今いる点の隣人のうち、クエリに最も近い 1 本』にだけ進み続ける単純な探索) が対数時間で任意のノードに到達できる（Kleinberg 2000 の小さな世界モデル）。Kleinberg の要諦は、<strong>その格子モデルではノード間距離 $r$ に対して確率 $\propto 1/r^d$ で長距離エッジを張るとき、そしてその時に限って</strong>、局所情報だけを見る貪欲アルゴリズムが $O(\log^2 N)$ ホップで目的地に着く、というものです。HNSW は確率的な階層と多様性を重視した近傍選択でこの直観を借りますが、Kleinberg の厳密な長距離エッジ分布を明示的にサンプリングするわけではありません。
