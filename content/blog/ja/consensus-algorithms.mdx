...
<strong>Election Restriction</strong>: 投票するノードは、候補者のログが自分のログ以上に新しい場合にのみ投票する（まず最後のログエントリの term、同じなら index を比較）。下のコミット規則と組み合わさることで Leader Completeness が守られ、コミット済みエントリを欠いたノードは後続 term の Leader になれない。

### ログ複製

1. Leader がクライアントからの書き込みをログに追記
2. `AppendEntries` RPC で全 Follower にログエントリを送信
3. Leader の現在 term のエントリについては、過半数が応答 → コミット（State Machine に適用）

Raft の安全性で特に重要なのは、Leader が<strong>過去 term のエントリを、レプリカ数だけを根拠にコミットしてはならない</strong>という規則だ。過去 term のエントリは、Leader が現在 term のエントリを少なくとも1つ過半数に複製してコミットしたときに、間接的にコミット済みになる。Log Matching は `AppendEntries` の `prevLogIndex` / `prevLogTerm` 検査で保たれる。同じ index・term のエントリを2つのログが含むなら、その前までのログも同一でなければならない。メンバーシップ変更は、旧構成と新構成で別々の Leader が選ばれないよう、重なり合う過半数（論文の joint consensus、実装によっては後続の one-server-at-a-time 方式）で行う。
...
Prepare・Commit フェーズではそれぞれ $2f+1$ のメッセージを集める。$n = 3f + 1$ なら、任意の2つのクォーラムは少なくとも $f+1$ 台で交差するため、$f$ 台がビザンチンでも交差部分に少なくとも1台の正しいノードが含まれる。
