...
ワークフローエンジンの各操作では、状態変更と配信意図を<strong>アトミック</strong>（全部成功するか全部失敗するか）に永続化する必要があります。

1. イベントストアにイベントを追記する
2. タスク投入メッセージを Outbox テーブルに記録する
3. タイマー登録メッセージを同じ Outbox テーブルに記録する
...
| スナップショット / リプレイ最適化 | `get_events_since()` | Sticky Execution（ワーカー上のインメモリキャッシュ。永続スナップショットではなく、ワーカー障害時はフルリプレイが行われる） | Extended Sessions / instance cache（インメモリのリプレイキャッシュで、永続スナップショットではない。対応は storage provider に依存） |
...
6. <strong>Outbox パターン</strong>——状態更新とメッセージ送信の意図をアトミックに永続化し、分散システムの整合性を確保する。後続のリレーは at-least-once 配信なのでコンシューマーには冪等性が必要。
