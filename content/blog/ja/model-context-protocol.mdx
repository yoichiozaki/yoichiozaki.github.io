...
> **仕様バージョンについて（追記）**
>
> MCP の仕様は `YYYY-MM-DD` 形式でリビジョン管理されており、この日付は「最後に後方互換性を破る変更をした日」を意味します。本記事の基準である `2025-06-18` はすでに過去のリビジョンであり、現行版は [`2026-07-28`](https://modelcontextprotocol.io/specification/2026-07-28/) です。概念の大半（Tools / Resources / Prompts、JSON-RPC 2.0、Streamable HTTP）はそのまま通用しますが、少なくとも次の 2 点は大きく変わっています。
>
> 1. **バージョン交渉のモデルがハンドシェイク型からリクエスト単位へ**。`2025-11-25` 以前は `initialize` で一度だけバージョンとケイパビリティを合意する方式でしたが、`2026-07-28` では各リクエストが `_meta` の `io.modelcontextprotocol/protocolVersion` でバージョンを宣言し、サーバーがリクエストごとに受諾・拒否します。バージョンとケイパビリティを事前に知りたいクライアント向けには必須 RPC の `server/discover` が用意されています。
> 2. **任意機能が「実験的機能」から正規の「拡張（Extensions）」に再編された**。拡張は `io.modelcontextprotocol/ui` のような `{vendor-prefix}/{name}` 形式の ID で識別され、**常に既定無効・明示的なオプトインが必要**です。後述の Tasks と MCP Apps は、現在はこの拡張メカニズムの上に乗っています。
>
> 以降の本文では、実際の JSON 例やヘッダ名は `2025-06-18` 時点のものとして読んでください。実装する際は必ず現行仕様を当たってください。
...
Tasks は **遅延結果取得とステータス追跡のための永続的な実行ラッパー** です。高コストな計算、ワークフロー自動化、バッチ処理、マルチステップ操作などのシナリオで、リクエストの実行状況を追跡できるようにします。

本記事の基準である `2025-06-18` 仕様では、Tasks はアーキテクチャ概要で言及されるだけの実験的機能でした。現行の `2026-07-28` 仕様では、Tasks は [MCP Tasks 拡張](https://modelcontextprotocol.io/extensions/tasks/overview) として正式な位置づけを得ており、**ポーリング、実行途中での入力受付（mid-flight input）、永続的なハンドル** を含む非同期タスク実行を定義しています。拡張である以上、クライアント・サーバー双方の明示的なオプトインが前提であり、片方が未対応の場合はコアの振る舞いへのフォールバックか、明示的なエラー返却が必要です。
...
詳細は [MCP Specification 2025-06-18](https://modelcontextprotocol.io/specification/2025-06-18) を参照してください。現行版との差分を確認する場合は [最新仕様](https://modelcontextprotocol.io/specification/latest) と [バージョニング方針](https://modelcontextprotocol.io/specification/versioning) もあわせて参照してください。
