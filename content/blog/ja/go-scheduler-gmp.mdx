...
- **Linux では cgroup の CPU 帯域制限を考慮する** — 制限が論理 CPU 数より小さければ、そちらを `GOMAXPROCS` の既定値にする。Kubernetes でいう **CPU limit** が対象で、**CPU requests は考慮されない**
- **全 OS で定期的に再評価する** — 利用可能な論理 CPU 数や cgroup 制限が変われば `GOMAXPROCS` を追従させる
