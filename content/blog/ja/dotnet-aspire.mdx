...
> **名称とバージョンについて（追記）**
>
> 本記事は Aspire がまだ「.NET Aspire」と呼ばれていた頃に書かれています。その後プロダクトは **「Aspire」へと改名**され、位置づけも .NET 専用から変わりました。現在の Aspire は AppHost を **C# または TypeScript** で記述でき、C# / JavaScript / Python / Go / Java / Rust / PowerShell など多言語のワークロードを一つのアプリケーションモデルでオーケストレートします。
>
> - ドキュメント: [aspire.dev](https://aspire.dev/)（旧: learn.microsoft.com/dotnet/aspire）
> - リポジトリ: [microsoft/aspire](https://github.com/microsoft/aspire)（旧: dotnet/aspire）
> - バージョン: 本稿執筆時の 9.x から進み、[`Aspire.Hosting.AppHost`](https://www.nuget.org/packages/Aspire.Hosting.AppHost) は 2026 年 9 月 10 日時点で <strong>13.5.3</strong> が最新
> - CLI: `aspire run` / `aspire agent init` などの専用 CLI が中心になり、AI コーディングエージェント向けに MCP サーバーや Skill ファイルも提供される
>
> 以下で解説する App Model、オーケストレーション、サービスディスカバリ、ヘルスチェック、テレメトリ統合といった<strong>設計の骨格は現在もそのまま通用します</strong>が、名称・パッケージバージョン・ドキュメント URL は最新のものに読み替えてください。
...
第3の柱である**ツーリング**は、Visual Studio や `aspire` CLI（C# AppHost では `dotnet run --project AppHost` も引き続き利用可能）、Azure Developer CLI (`azd`) によるエンドツーエンドの開発体験を提供します。プロジェクトの作成からローカルデバッグ、クラウドデプロイまでを一貫して支援します。
...
Dashboard はスタンドアロンの Docker コンテナとしても利用可能です（MCR では 2026 年 9 月時点で `13.5` タグが公開されています）。
...
```bash
# マニフェストの生成（C# AppHost は dotnet run でも起動できるが、現在は aspire CLI が中心）
...
| `AddRabbitMQ()` | RabbitMQ コンテナまたは明示的な発行先（Service Bus を使う場合は `AddAzureServiceBus()` などを選ぶ） |
...
Aspire はリリースサイクルが速く、.NET 9 と同時にリリースされた Aspire 9 では、コミュニティのフィードバックを反映した多くの改善が行われています。特に注目すべきは、開発体験のさらなる向上を目指した新機能群です（このリリースペースはその後さらに加速し、バージョン番号は 13.x まで進んでいます。以下は「9 世代で何が入ったか」のスナップショットとして読んでください）。
...
// コンテナの永続化（再起動しても維持）
var postgres = builder.AddPostgres("pg")
    .WithLifetime(Lifetime.Persistent) // デフォルトは Lifetime.Session
    .AddDatabase("catalogdb");

// Lifetime.Session: AppHost 停止時にコンテナも停止
// Lifetime.Persistent: AppHost 停止後もコンテナが残る（高速な再起動）
```
...
- [Aspire ソースコード (GitHub)](https://github.com/microsoft/aspire)
- [Aspire サンプル集 (GitHub)](https://github.com/dotnet/aspire-samples)
- [Microsoft.Extensions.ServiceDiscovery ソースコード](https://github.com/microsoft/aspire/tree/main/src/Microsoft.Extensions.ServiceDiscovery)
- [Aspire Dashboard](https://aspire.dev/dashboard/)
- [Aspire インテグレーション一覧](https://aspire.dev/integrations/overview/)
- [Aspire.Hosting.AppHost (NuGet)](https://www.nuget.org/packages/Aspire.Hosting.AppHost) — 現行バージョンの確認用
