...
pytest は `describe`/`it` のような DSL を使いません。標準的な関数テストは<strong>関数名の規約</strong>（`test_` で始まる関数）で識別されます。ただし実際の pytest は `Test*` クラス、`unittest.TestCase`、設定による `python_files` / `python_classes` / `python_functions` の変更もサポートします。これは Python の `inspect` モジュールと組み合わさることで、非常に強力なディスカバリを実現します。
...
この `try-except` モデルのもう一つの利点は、<strong>予期しない通常の例外（`TypeError`、`KeyError` など）もキャッチする</strong>ことです。アサーション失敗だけでなく、テスト中に発生した多くの例外を安全に捕捉し、残りのテストを続行できます。ただし `KeyboardInterrupt` や `SystemExit` のような `BaseException` 系は、ランナーが中断・終了シグナルとして別扱いするのが一般的です。この堅牢性こそがテストランナーに求められる基本的な性質です。
...
```text
Go testing の特徴:
  - テストディスカバリ: `cmd/go` が `Test*` / `Benchmark*` / `Fuzz*` 関数からテスト用バイナリを生成
  - アサーション: 標準ライブラリにはない（if + t.Error が基本）
  - 並列化: t.Parallel() で明示的にオプトイン
  - ベンチマーク: Benchmark* 関数で統合
  - ファジング: Fuzz* 関数で統合（Go 1.18+）
  - 並行コードのテスト: testing/synctest で仮想時間を使う（Go 1.25+）
```

最後の [`testing/synctest`](https://go.dev/pkg/testing/synctest) は、並行コードのテストにつきまとう「`time.Sleep` で待つしかない → 遅い上に Flaky」という問題に対する回答です。`synctest.Test` はテスト関数を隣接した「バブル」の中で実行し、その中では `time` パッケージが仮想クロックを使います。バブル内の全 goroutine がブロックした瞬間に時計が瞬時に進むため、「1 時間後のタイムアウト」を実時間 0 秒で決定的にテストできます。Go 1.24 で `GOEXPERIMENT=synctest` として導入され、Go 1.25 で正式提供となりました。
...
```text
Jest:
  - ディスカバリ: 既定では `**/__tests__/**/*.[jt]s?(x)` と `**/?(*.)+(spec|test).[tj]s?(x)` に相当するパターン
...
Vitest:
  ...
  - ESM ネイティブ対応
  - pool 設定に応じて worker threads / child processes などで並列化
```
