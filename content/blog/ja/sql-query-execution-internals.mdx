...
`random_page_cost = 4.0` は「物理的なランダム読みが順次読みの4倍しか高くない」と PostgreSQL が考えている、という意味ではありません。公式ドキュメントにもある通り、インデックス経由のランダムアクセスの多くがキャッシュに載る前提を含めて、実ハードウェア比より低い既定値になっています。SSD 中心、あるいはほぼ全体がキャッシュに載るワークロードでは、実測に合わせて 1.1〜2 程度へ下げる（ランダムアクセスのペナルティが実質ない場合だけ `seq_page_cost` と同じにする）のが一般的なチューニングです。この1つの定数が、インデックススキャンとシーケンシャルスキャンの分岐点を動かします。
...
| `read=9976` | 第16章 — 共有バッファへストレージから読み込んだブロックが 9,976 個。必ずしも 1 ブロック 1 システムコールではない |
...
`EXPLAIN (ANALYZE, BUFFERS)` の出力が、これで違って見えるはずです。`Hash Join` の下にぶら下がる `Hash` はブロッキング演算子であり、`cost=2387.50..25687.50` の左の数字は第7章の起動コストであり、`rows=55000` は第6章の推定であり、`Buffers: shared hit=1024 read=9976` は第14章のバッファプールの挙動です。ただし `read=9976` はストレージから共有バッファへ読んだブロック数であり、`pread(2)` の呼び出し回数と必ず1対1に対応するわけではありません。
