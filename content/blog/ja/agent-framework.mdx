...
> **追記（2026-08 時点）**：Agent Framework はその後安定版に到達し、`Microsoft.Agents.AI` は **1.x 系列（本稿更新時点で 1.19.0）** の正式リリースとして NuGet で配布されている。また、対応言語に **Go**（[microsoft/agent-framework-go](https://github.com/microsoft/agent-framework-go)、public preview）が加わった。公式ドキュメントは現在、フレームワークを **Agents / Harness Agent / Workflows / Integrations** の 4 つの柱で説明しており、計画・TODO・コンテキスト圧縮・ツール承認を標準搭載した [Harness Agent](/ja/blog/agent-harness-doc-reviewer) が新たな柱として加わっている。
