...
> **注意**: Handoff API（`CreateHandoffBuilderWith` / `HandoffWorkflowBuilder`）は `[Experimental("MAAIW001")]` としてマークされている。利用時はコンパイラ警告 `MAAIW001` を抑制する必要がある。API は今後変更される可能性がある。（2026 年 8 月再確認：Agent Framework 本体は安定版 1.x に到達したが、Handoff は**依然として実験的**のままだ。なお、旧名の `HandoffsWorkflowBuilder`（s 付き）は `[Obsolete]` になっており、`HandoffWorkflowBuilder` を使うこと。）
