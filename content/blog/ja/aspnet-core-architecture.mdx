...
`builder.Build()` で `WebApplication` が構築される過程で、登録されたミドルウェアが<strong>逆順</strong>にチェーンされます。これは<strong>ロシアンドール（マトリョーシカ人形）</strong>モデルと呼ばれます。
...
明示的に属性を指定することで、以下のソースからデータを取得できます。なお、属性を指定しない場合の既定の検索順序は、フォームフィールド → リクエストボディ（`[ApiController]` が付いたコントローラー）→ ルートデータ → クエリ文字列 → アップロードファイルです。ルートデータとクエリ文字列は主に単純型、アップロードファイルは `IFormFile` / `IEnumerable<IFormFile>` にだけ使われます。
...
| インターフェース | 用途 | リロード対応 |
|----------------|------|------------|
| `IOptions<T>` | Singleton。初回アクセス時に値を作成してキャッシュ | 不可 |
| `IOptionsSnapshot<T>` | Scoped。スコープ（Web では通常リクエスト）ごとに値を再計算 | 可。ただし Singleton には注入しない |
| `IOptionsMonitor<T>` | Singleton。現在値と変更通知を管理 | 可（`OnChange` コールバック） |
...
graph TD
    Ex["例外発生"]
    Ex --> Filter["Exception Filter<br/>(MVC アクション内)"]
    Ex --> MW["ExceptionHandler / DeveloperExceptionPage<br/>Middleware（パイプライン上流）"]
    Filter -->|処理した| Result1["カスタムエラーレスポンス"]
    Filter -->|未処理なら再スロー| MW
    MW -->|処理した| Result2["エラーページ / Problem Details"]
    MW -->|処理できない| Generic["500 Internal Server Error"]
```
