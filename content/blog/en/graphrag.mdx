...
[GraphRAG](https://github.com/microsoft/graphrag) is a RAG methodology developed by Microsoft Research. It uses LLMs to build a **knowledge graph** from unstructured text and leverages that graph structure to enhance context at query time. As of 2026-09-10, the `microsoft/graphrag` repository describes itself as largely in maintenance mode rather than an actively expanding product SDK; the methodology and documentation remain useful, but package APIs and configuration should be checked against the current docs before production use.
...
First, create a new console project and add the current Agent Framework/OpenAI packages. As of the 2026-09-10 Agent Framework quickstart, C# examples use `Microsoft.Agents.AI` together with the OpenAI .NET SDK; Azure OpenAI authentication uses `Azure.Identity` plus `System.ClientModel`. The later snippets in this article are intentionally illustrative GraphRAG-style code, so verify exact Agent Framework APIs before copying them into production.
...
We discuss community detection from Phase 4 of the GraphRAG indexing pipeline. The official GraphRAG uses Python's `graspologic` library to run Leiden; the C# code below is only a toy local-move detector to illustrate why graph clustering matters, not a faithful Leiden implementation.

The main processing flow is:

1. Build an adjacency list from entity relationships
2. Initialize each entity in its own community
3. For each entity, move it to the neighboring community with the highest toy neighbor-count score
4. Repeat until no further improvements are found

```csharp
// Services/CommunityDetector.cs
// Toy community detector for exposition; not a complete Leiden implementation

/// <summary>
/// Toy hierarchical community detection for exposition.
/// GraphRAG uses graspologic (Python); this C# example
/// only demonstrates a toy local-move clustering heuristic.
/// </summary>
public class CommunityDetector
{
        ...
        int maxCommunitySize = 10)
    {
        ...
        // 3. Toy local-move loop (real Leiden optimizes modularity/CPM more carefully)
        bool improved;
        do
        {
            improved = false;
    ...
    {
        // Toy score: count neighbors in each neighboring community
...
The implementation above is a toy local-move community detector for exposition, not a correct Leiden implementation. A full Leiden implementation would include:
...
For production use, prefer the official Python GraphRAG/graspologic pipeline for indexing and consume its Parquet outputs from .NET, or use a graph library/runtime that provides a maintained Leiden implementation.
...
LazyGraphRAG:      NLP-based indexing, all LLM processing deferred to query time (reported by Microsoft Research as vector-RAG-level indexing cost, roughly 0.1% of standard GraphRAG in their experiments)
```
...
GraphRAG behavior is controlled via a YAML configuration file (`settings.yaml`). The `graphrag init` command generates a template.

```yaml
# settings.yaml — Key configuration parameters (generated by graphrag init)
completion_models:
