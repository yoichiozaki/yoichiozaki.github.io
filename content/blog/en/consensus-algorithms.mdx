...
<strong>Election Restriction</strong>: A voting node only grants its vote if the candidate's log is at least as up-to-date as its own (compare last log term first, then last log index). Together with the commitment rule below, this preserves Leader Completeness: a node missing a committed entry cannot win a later election.

### Log Replication

1. The Leader appends client writes to its log
2. Sends log entries to all Followers via `AppendEntries` RPC
3. For entries from the leader's current term, once a majority responds → commit (apply to state machine)

Raft's subtle safety rule is that a leader <strong>must not</strong> commit an entry from a previous term merely by counting replicas. Older entries become committed only indirectly, when the leader first commits at least one entry from its current term on a majority. Log Matching is enforced by `prevLogIndex` / `prevLogTerm` checks in `AppendEntries`: if two logs contain an entry with the same index and term, all preceding entries are identical. Membership changes are handled with overlapping majorities (joint consensus in the paper; many production systems use Raft's later one-server-at-a-time variant) so old and new configurations cannot choose different leaders.
...
The Prepare and Commit phases each require collecting $2f+1$ messages. With $n = 3f + 1$, any two such quorums intersect in at least $f+1$ replicas, so at least one correct replica is shared even if $f$ replicas are Byzantine.
