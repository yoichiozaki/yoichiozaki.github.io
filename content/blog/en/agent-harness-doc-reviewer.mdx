...
> **Update (as of August 2026)**: at the time of writing, `HarnessAgent` was an experimental (preview) API that emitted the diagnostic warning `MAAI001` at compile time. That has since been lifted — `HarnessAgent` is now a <strong>fully released API shipped in the stable `Microsoft.Agents.AI.Harness` package (1.19.0 at the time of this update)</strong>, and the official documentation lists the Harness as one of Agent Framework's four pillars (Agents / **Harness Agent** / Workflows / Integrations). The `--prerelease` flags and `MAAI001` suppression in the code below are no longer required for the harness itself. Note that `MAAI001` still exists as the framework-wide experimental diagnostic, so you will still need to suppress it if you use A2A, Mem0, DevUI, declarative agents, shell execution, and similar features.
...
At the time of writing, `HarnessAgent` was experimental (`MAAI001`), in contrast to the Copilot SDK being GA (generally available). <strong>That gap has since closed.</strong> `HarnessAgent` now ships in the stable `Microsoft.Agents.AI.Harness` package (1.19.0 at the time of this update) and carries no `[Experimental]` attribute.
