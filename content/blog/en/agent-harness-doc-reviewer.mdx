...
6. <strong>Cons and caveats</strong> — API stability, non-determinism, security
...
> <strong>Update (as of September 10, 2026)</strong>: at the time of writing, `HarnessAgent` was an experimental (preview) API that emitted the diagnostic warning `MAAI001` at compile time. That has since been lifted — `HarnessAgent` is now a <strong>fully released API shipped in the stable `Microsoft.Agents.AI.Harness` package (1.20.0 at the time of this update)</strong>, and the official documentation lists the Harness as one of Agent Framework's foundational concepts (Agents / Workflows / <strong>Agent Harness</strong>). The `--prerelease` flags and `MAAI001` suppression in the code below are no longer required for the harness itself. Note that `MAAI001` still exists as the framework-wide experimental diagnostic, so you will still need to suppress it if you use file memory, A2A, Mem0, DevUI, declarative agents, shell execution, and similar features.
...
Add the dependencies. At the time of writing the Agent Harness was experimental, so `--prerelease` was required (there is now a stable `Microsoft.Agents.AI.Harness`, so the flag is no longer needed for the harness itself).
...
At the time of writing `HarnessAgent` was an experimental API, so we suppress the warning in the `.csproj` (in current versions, suppressing `MAAI001` is not needed for the harness alone — only if you use other experimental features).
...
A document review needs none of the harness's default web search, file memory, or skill discovery. Arbitrary file I/O is enabled only when you pass a `FileAccessStore`, so this example simply does not configure one. If anything, since we read untrusted documents, the <strong>attack surface should be minimized.</strong> So we drop those with `Disable*` flags and keep <strong>only the planning features (TodoProvider, AgentModeProvider) and our own tools.</strong>
...
AIAgent CreateReviewer(IChatClient client, Action<string, string> emit) =>
    client.AsHarnessAgent(new HarnessAgentOptions
    {
        ...
        DisableToolAutoApproval = true,   // no approval-required tools: leave out the auto-approval middleware

        // Keep TodoProvider and AgentModeProvider (plan/execute) → leverage planning
        // FileAccessStore and ShellExecutor are not set → arbitrary file I/O and shell execution are impossible from the start
...
Since `FileAccessStore` and `ShellExecutor` are not set, <strong>arbitrary file I/O and shell execution are never configured in the first place.</strong> We set `DisableToolAutoApproval = true` because this server uses no approval-required tools and narrows capability to its own tools. Instead, we secure safety by <strong>narrowing the provided capabilities to only safe ones</strong> — that's the harness-style defense.

Be clear about what these flags do and don't cover: the `Disable*` flags switch off the harness's <strong>built-in</strong> providers (the `HostedWebSearchTool`, `FileMemoryProvider`, and so on). `FileAccessProvider` is added only when `FileAccessStore` is explicitly supplied. They do <strong>not</strong> touch your own tools — `save_review` still writes and `check_links` still fetches, because they're your `AIFunction`s, not harness providers. Just as in the Copilot SDK version, the real guardrails for custom tools live <strong>inside the tool</strong> (the path-traversal check, the timeout, an allowlist).

> <strong>Capability minimization is the harness's line of defense.</strong> If you pass a `ShellExecutor` or `FileAccessStore`, or drop `DisableWebSearch`, the agent becomes more powerful but its attack surface widens sharply. As long as you have it read untrusted input (the documents under review), "what you don't build in" is the center of the security design.
...
### API Stability (It Was an Experimental API at the Time of Writing)

At the time of writing, `HarnessAgent` was experimental (`MAAI001`), in contrast to the Copilot SDK being GA (generally available). <strong>That gap has since closed.</strong> `HarnessAgent` now ships in the stable `Microsoft.Agents.AI.Harness` package (1.20.0 at the time of this update) and carries no `[Experimental]` attribute.

That said, "the harness itself is stable" is not the same as "everything around it is stable." On the .NET side, A2A, Mem0, DevUI, declarative agents, and shell tools are still covered by `MAAI001`; on the Python side, background agents, file access, and looping remain experimental, and shell tooling still comes from a pre-release package. For production adoption it is worth checking stability feature by feature rather than framework-wide.
...
- <strong>Prompt injection</strong> — the defense against malicious instructions inside a document centers, in the harness, on <strong>capability minimization.</strong> The more you enable `FileAccessStore` / web search or pass a `ShellExecutor`, the greater the damage an injection can do.
...
- <strong>Choosing an approval model</strong> — this implementation is a non-interactive server, so we set `DisableToolAutoApproval = true` and substituted capability narrowing. For interactive use, you could instead enable tool approval and let a human decide. <strong>"Narrow by configuration" vs. "approve at runtime"</strong> is a choice you make based on the use case.

### .NET-Centric

The most mature implementation is the .NET one. The Agent Framework harness is also officially offered in Python (`create_harness_agent`; the repo has a [Python harness sample](https://github.com/microsoft/agent-framework/tree/main/python/samples/02-agents/harness)), and Agent Framework itself now has a Go implementation as well (public preview). Go, however, has no packaged harness yet — you compose the individual packages yourself. This article used the verified C#. For multilingual teams, language choice can be a constraint.
...
| Option | When it fits |
|--------|--------------|
| <strong>GitHub Copilot SDK</strong> | Move fast in many languages (especially Node/TS). Within the GitHub ecosystem/subscription. You want fine-grained runtime permission control. |
...
4. <strong>The cost is delegation-common</strong> — the delegation-common costs of non-determinism, cost, and security still apply here. The "the harness is an experimental API" constraint noted at the time of writing has since been lifted; it now ships as a stable release.
