...
> **Note**: The Handoff API (`CreateHandoffBuilderWith` / `HandoffWorkflowBuilder`) is marked `[Experimental("MAAIW001")]`. You must suppress compiler warning `MAAIW001` to use it. The API may change in future releases. (Rechecked in August 2026: this is **still** experimental even though Agent Framework itself has reached a stable 1.x release. Separately, the older `HandoffsWorkflowBuilder` — with an "s" — is now `[Obsolete]`; prefer `HandoffWorkflowBuilder`.)
