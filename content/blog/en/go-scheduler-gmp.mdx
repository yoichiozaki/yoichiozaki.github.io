...
- **On Linux the runtime accounts for the cgroup CPU bandwidth limit** — if the limit is lower than the number of logical CPUs, that lower value becomes the `GOMAXPROCS` default. This corresponds to the Kubernetes **CPU limit**; **CPU requests are not considered**.
- **On all OSes the value is re-evaluated periodically** — `GOMAXPROCS` follows changes in available logical CPUs or in the cgroup limit.
