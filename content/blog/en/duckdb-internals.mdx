...
sequenceDiagram
    ...
    Note over Chk: WAL exceeds checkpoint_threshold (16 MiB default)
...
Individual passes can be turned off with a comma-separated list (`SET disabled_optimizers='join_order'`), and `SELECT * FROM duckdb_optimizers();` lists every valid name — so you can measure exactly what each one buys.
