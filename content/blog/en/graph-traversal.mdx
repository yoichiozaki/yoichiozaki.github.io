...
Why is this useful? On a **directed acyclic graph (DAG)**, the reverse of DFS finish times gives **topological sort**. With the convention that an edge $u \to v$ means "$u$ must come before $v$," every edge in a DAG satisfies $\mathrm{finish}(u) > \mathrm{finish}(v)$, so reversing finish times puts $u$ before $v$. If the graph has a cycle, topological order does not exist and DFS must detect/report that cycle.

Build systems use this idea after choosing a dependency-edge convention. If edges point from dependency to dependent, "libC → libB → libA" yields the build order C → B → A.
...
- Cycle detection (in directed graphs, did we encounter a vertex currently on the recursion stack? In undirected graphs, did we encounter a visited vertex other than the parent?)
...
Reasoning from DFS: "For a DAG, the reverse of DFS finish times gives the correct order." With edges directed from prerequisite to dependent, every edge $u \to v$ should place $u$ before $v$; reverse finish time has exactly that property. If a cycle is found, no topological order exists.
