...
> **A note on specification versions (added later)**
>
> MCP revisions are identified as `YYYY-MM-DD`, where the date marks the last time a backwards-incompatible change was made. The `2025-06-18` revision this article is built on is now a past revision; the current one is [`2026-07-28`](https://modelcontextprotocol.io/specification/2026-07-28/). Most of the conceptual material below still holds — Tools / Resources / Prompts, JSON-RPC 2.0, Streamable HTTP — but at least two things changed substantially:
>
> 1. **Version negotiation moved from a handshake to per-request.** In `2025-11-25` and earlier, the version and capabilities were agreed once during `initialize`. In `2026-07-28`, every request declares its version through `io.modelcontextprotocol/protocolVersion` in `_meta`, and the server accepts or rejects each request independently. Clients that want to know versions and capabilities up front can call `server/discover`, a mandatory RPC.
> 2. **Optional features were reorganized from ad-hoc "experimental" flags into first-class Extensions.** Extensions are identified as `{vendor-prefix}/{name}` (for example `io.modelcontextprotocol/ui`) and are **always disabled by default, requiring explicit opt-in**. Tasks and MCP Apps, both discussed below, now sit on top of this extension mechanism.
>
> Read the concrete JSON payloads and header names below as being specific to `2025-06-18`, and check the current specification before implementing against it.
...
Tasks provide **durable execution wrappers for deferred result retrieval and status tracking**. They enable tracking the execution status of requests for scenarios like expensive computations, workflow automation, batch processing, and multi-step operations.

In the `2025-06-18` specification this article is based on, Tasks were only an experimental feature mentioned in the architecture overview. In the current `2026-07-28` specification, Tasks have graduated into the official [MCP Tasks extension](https://modelcontextprotocol.io/extensions/tasks/overview), which defines asynchronous task execution including **polling, mid-flight input, and durable handles**. Being an extension, it requires explicit opt-in from both client and server; if one side does not support it, the other must either fall back to core protocol behavior or return an explicit error.
...
See the [MCP Specification 2025-06-18](https://modelcontextprotocol.io/specification/2025-06-18) for details. To check what has changed since, see the [current specification](https://modelcontextprotocol.io/specification/latest) and the [versioning policy](https://modelcontextprotocol.io/specification/versioning).
