...
> **Update (as of August 2026)**: Agent Framework has since reached stable status — `Microsoft.Agents.AI` ships on NuGet as a released **1.x line (1.19.0 at the time of this update)**. **Go** has also joined the supported languages ([microsoft/agent-framework-go](https://github.com/microsoft/agent-framework-go), public preview). The official documentation now describes the framework in terms of four pillars — **Agents / Harness Agent / Workflows / Integrations** — adding the [Harness Agent](/en/blog/agent-harness-doc-reviewer), which ships planning, todos, context compaction, and tool approval out of the box.
