...
<strong>Combiner optimization</strong>: To reduce network transfer volume, a <strong>Combiner</strong> function can run on the Map side. It is often the same code as Reduce, but it must be safe when applied zero, one, or multiple times to subsets of a mapper's output, and its output type must match the intermediate key/value type. For Word Count, instead of sending `(hello, 1), (hello, 1), (hello, 1)` over the network, the Combiner aggregates to `(hello, 3)` before sending.
...
Combiners are valid only when partial aggregation preserves the final result even if the framework runs the combiner zero, one, or multiple times. Associative and commutative operations such as sum, max, and min work; average cannot be used directly unless you combine numerator and denominator (or sum and count) separately.
...
- <strong>Atomic rename / commit semantics</strong> for final Reduce output; intermediate Map output stays on local worker disks, not in GFS
- **Optimized for large files** (small files are problematic)

This design philosophy later inspired <strong>HDFS</strong> (Hadoop Distributed File System), an open-source GFS-like file system developed in the Hadoop ecosystem with major Yahoo! support.
...
In 2006, Doug Cutting and Mike Cafarella developed Apache Hadoop as open-source implementations inspired by Google's MapReduce and GFS papers, with major Yahoo! support. The HDFS + Hadoop MapReduce combination enabled petabyte-scale data processing for companies beyond Google.
