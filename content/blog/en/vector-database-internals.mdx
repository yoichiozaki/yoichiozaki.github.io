...
<strong>Zero float multiplications per distance</strong> after the lookup tables are built; with $m=64$ each stored code needs 64 byte-indexed table lookups and additions of the table values. SIMD fast-scan kernels such as FAISS `pq4_fast_scan` can scan very large batches quickly, but the exact throughput depends on code size, CPU SIMD width, memory layout, and recall settings.

<strong>In short</strong>: PQ turns distance computation from "$d$-dimensional subtraction and squaring" into "summing values from a precomputed table", and that substitution is the whole reason PQ is orders of magnitude faster.
...
1. <strong>Navigable Small World (NSW)</strong> — a graph with both short and long-range edges supports greedy routing (at every step, move to whichever neighbour of the current node is closest to the query — a purely local rule) to any node in logarithmic hops (Kleinberg's small-world model). Kleinberg's key result is that when long-range edges are drawn with probability $\propto 1/r^d$ in the node-distance $r$ — <strong>and only in that case for that grid model</strong> — a purely local greedy algorithm reaches its target in $O(\log^2 N)$ hops. HNSW borrows this intuition by combining probabilistic layers with diversity-based neighbor selection; it does not explicitly sample long-range edges from Kleinberg's exact distribution.
