...
Fixed-priority scheduling always chooses the highest-priority runnable task. In a simple implementation, low-priority tasks can starve, so aging or bounded priority boosts are needed. <strong>Priority inversion</strong> is a different failure mode: a low-priority task holding a shared lock can block a high-priority task while medium-priority work preempts the lock holder. Priority inheritance temporarily boosts the lock holder to the waiting task's priority. Priority-ceiling protocols instead use each resource's ceiling priority to restrict or raise lock holders, limiting inversion and deadlock.
...
In the same independent periodic-task, implicit-deadline model with idealized preemption and switching cost, EDF is schedulable exactly up to total utilization $U\le1$, i.e. 100% of one CPU.
...
Earliest Eligible Virtual Deadline First (EEVDF) was proposed in 1995, and Linux began moving to Peter Zijlstra's 2023 EEVDF design in version 6.6. As of 2026, describing mainline Linux's normal fair class as still using the old CFS “pick minimum `vruntime`” rule by default is no longer accurate: the default selection rule is EEVDF, built in the same virtual-time lineage. EEVDF expresses how far a task is behind its fair share as `lag`; among tasks with `lag >= 0`, it chooses the earliest virtual deadline. This preserves proportional sharing while making latency-sensitive tasks with shorter requested slices easier to express. CFS and EEVDF belong to the same virtual-time family, but EEVDF makes eligibility and deadline explicit in the selection rule.
