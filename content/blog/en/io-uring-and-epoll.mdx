...
A <strong>syscall</strong>, in this context, is the privileged mode switch a user-space program uses to enter the kernel. It involves a trap instruction and a CPU-mode transition, so it costs orders of magnitude more than a plain function call. Treat figures such as 200–500 ns for a simple syscall and < 10 ns for a regular call as order-of-magnitude examples; the exact cost depends heavily on CPU, kernel, mitigations, and cache/TLB state. KPTI (described later) can make this worse still.
...
- <strong>`EPOLLEXCLUSIVE`</strong> (Linux 4.5+) — reduce thundering-herd wakeups by marking the epoll wait-queue entry exclusive; the kernel wakes one or a small subset of exclusive waiters rather than every waiter.
...
The key property: <strong>many SQEs can be submitted in a single `io_uring_enter`</strong>. One syscall can send 1000 reads, amortising the syscall cost to nearly zero. As a back-of-the-envelope illustration rather than a portable benchmark: an epoll-shaped loop can pay two syscalls per I/O, while batching 1000 io_uring requests spreads one submission syscall across the whole batch.
...
Set `IORING_SETUP_SQPOLL` and the kernel spawns a <strong>dedicated thread</strong> that polls SQ. While that thread is awake and the ring stays busy, userspace can submit and reap I/O through shared memory without calling `io_uring_enter`; if the SQ thread goes idle and sets `IORING_SQ_NEED_WAKEUP`, userspace must make a wakeup `io_uring_enter` call. In call-centre terms, SQPOLL is the mode where an agent sits at the shared counter, picking up request slips as soon as you place them down, until the counter has been idle long enough for the agent to nap.
...
For `O_DIRECT` block I/O on filesystems and devices that support polling, busy-polling the block layer for completion can beat interrupt-driven delivery at NVMe latencies (µs range). Applications still reap polled completions via `io_uring_enter`.
...
> <strong>TL;DR</strong>: powerful but also the source of many CVEs, and `seccomp` can't filter at opcode granularity. Production use demands current LTS kernels, decent observability, and awareness that some hardened platforms restrict or disable it by policy.
...
- As a result, <strong>some hardened platforms, notably Android and ChromeOS, disable or restrict `io_uring` by default</strong>; general-purpose server distributions vary, so check the platform policy.
- Since RHEL 9.5, `sysctl kernel.io_uring_disabled=1/2` can outright disable it or allow root only.

In production: stay on current LTS kernels, use `seccomp` to block or restrict the `io_uring_*` syscalls where appropriate, and use `IORING_REGISTER_RESTRICTIONS` or LSM-based controls when you need opcode-level restrictions.
...
- Per-ring details are exposed through `/proc/<pid>/fdinfo/<ring-fd>` on kernels that include the io_uring fdinfo hooks; tracepoints are usually more useful for live debugging.
