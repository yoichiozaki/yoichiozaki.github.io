...
`random_page_cost = 4.0` encodes the HDD-era assumption that random reads cost four times a sequential read. On SSDs the measured ratio is closer to 1.1–2, so lowering `random_page_cost` to around 1.1 is a standard tuning step. That single constant moves the break-even point between index scans and sequential scans.
...
| `read=9976` | Ch. 16 — 9,976 `pread(2)` calls |
...
`EXPLAIN (ANALYZE, BUFFERS)` should read differently now. The `Hash` node under `Hash Join` is a blocking operator; the left-hand number in `cost=2387.50..25687.50` is chapter 7's startup cost; `rows=55000` is the chapter 6 estimate; `Buffers: shared hit=1024 read=9976` is chapter 14's buffer pool at work; and each of those 9,976 reads is a chapter 16 `pread(2)`.
