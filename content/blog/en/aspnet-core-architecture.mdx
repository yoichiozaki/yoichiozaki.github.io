...
When `builder.Build()` constructs the `WebApplication`, registered middleware are chained in <strong>reverse order</strong>. This is known as the <strong>Russian Doll (Matryoshka)</strong> model.
...
By specifying attributes explicitly, data can be retrieved from the following sources. Note that the default search order without explicit attributes is: form fields → request body (for controllers with `[ApiController]`) → route data → query string → uploaded files. Route data and query string values are mainly used for simple types, and uploaded files bind only to `IFormFile` / `IEnumerable<IFormFile>`.
...
|-----------|----------|---------------|
| `IOptions<T>` | Singleton. Value is created on first access and cached | No |
| `IOptionsSnapshot<T>` | Scoped. Recomputes values per scope (usually per request in web apps) | Yes. Don't inject it into singletons |
| `IOptionsMonitor<T>` | Singleton. Manages current value and change notifications | Yes (`OnChange` callback) |
...
graph TD
    Ex["Exception Thrown"]
    Ex --> Filter["Exception Filter<br/>(inside MVC action)"]
    Ex --> MW["ExceptionHandler / DeveloperExceptionPage<br/>Middleware (early pipeline)"]
    Filter -->|Handled| Result1["Custom error response"]
    Filter -->|Rethrown if unhandled| MW
    MW -->|Handled| Result2["Error page / Problem Details"]
    MW -->|Unhandled| Generic["500 Internal Server Error"]
```
