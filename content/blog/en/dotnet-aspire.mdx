...
> **A note on the name and versions (added later)**
>
> This article was written while Aspire was still called ".NET Aspire." The project has since been **renamed to simply "Aspire"** and has stopped being .NET-only. Today the AppHost can be authored in **C# or TypeScript**, and it orchestrates workloads written in C#, JavaScript, Python, Go, Java, Rust, PowerShell, and more, all within a single application model.
>
> - Documentation: [aspire.dev](https://aspire.dev/) (previously learn.microsoft.com/dotnet/aspire)
> - Repository: [microsoft/aspire](https://github.com/microsoft/aspire) (previously dotnet/aspire)
> - Version: the 9.x line described here has moved on — [`Aspire.Hosting.AppHost`](https://www.nuget.org/packages/Aspire.Hosting.AppHost) is at <strong>13.5.3</strong> as of September 10, 2026
> - CLI: a dedicated `aspire` CLI (`aspire run`, `aspire agent init`, …) is now central, and Aspire also ships an MCP server and skill files for AI coding agents
>
> The <strong>structural ideas covered below — the App Model, orchestration, service discovery, health checks, and telemetry integration — still apply</strong>. Just mentally substitute the current names, package versions, and documentation URLs as you read.
...
The third pillar, **Tooling**, provides end-to-end developer experience through Visual Studio, the `aspire` CLI (C# AppHosts can still use `dotnet run --project AppHost`), and the Azure Developer CLI (`azd`), supporting everything from project creation and local debugging to cloud deployment.
...
The Dashboard is also available as a standalone Docker container (MCR publishes a `13.5` tag as of September 2026):
...
```bash
# Generate manifest (C# AppHosts can still be started with dotnet run, but the current workflow centers on the aspire CLI)
aspire publish --publisher manifest --output-path manifest.json
```
...
| `AddRabbitMQ()` | RabbitMQ container or explicit publisher target (use `AddAzureServiceBus()` or similar when the Azure target is Service Bus) |
...
Aspire has a fast release cadence. Aspire 9, released alongside .NET 9, incorporates many improvements reflecting community feedback. The most notable new features focus on further improving the developer experience. (That cadence has only accelerated since — the version number is now up in the 13.x range — so read this section as a snapshot of what landed in the 9 generation.)
...
var postgres = builder.AddPostgres("pg")
    .WithLifetime(Lifetime.Persistent) // Default is Lifetime.Session
    .AddDatabase("catalogdb");

// Lifetime.Session: container stops when AppHost stops
// Lifetime.Persistent: container survives AppHost shutdown (faster restarts)
```
...
- [Aspire Official Documentation](https://aspire.dev/)
- [Aspire Source Code (GitHub)](https://github.com/microsoft/aspire)
- [Aspire Samples (GitHub)](https://github.com/dotnet/aspire-samples)
- [Microsoft.Extensions.ServiceDiscovery Source Code](https://github.com/microsoft/aspire/tree/main/src/Microsoft.Extensions.ServiceDiscovery)
- [Aspire Dashboard](https://aspire.dev/dashboard/)
- [Aspire Integrations Overview](https://aspire.dev/integrations/overview/)
- [Aspire.Hosting.AppHost (NuGet)](https://www.nuget.org/packages/Aspire.Hosting.AppHost) — to check the current version
